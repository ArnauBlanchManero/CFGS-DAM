...
Book * pedir_libro(Book * direction_to_send){
	...
	float new_price;
	printf("Escribe los datos del nuevo libro:\n");
	printf("ID: ");
	scanf(" %d", &new_id);
	printf("Título: ");
	scanf(" %[^\n]", new_title);
	...
	printf("Precio: ");
	scanf(" %f", &new_price);
	printf("Género (0 -> FICTION, 1 -> NON_FICTION, 2 -> POETRY, 3 -> THEATER, 4 -> ESSAY): ");
	scanf(" %d", &new_gender);
	printf("Cantidad: ");
